...
- (void)regisetButtonAction{
    ...
    [self.checkCodeField addTarget:self action:@selector(textChange:) forControlEvents:(UIControlEventEditingChanged)];
    [self.passwordField addTarget:self action:@selector(textChange:) forControlEvents:(UIControlEventEditingChanged)];
    self.registerBtn.backgroundColor = kUIColorFromRGB(0xb2b2b2);
    ...
    if (self.hiddenPassword) {
        [self.hiddenPassword addTarget:self action:@selector(hiddenPasswordAction:) forControlEvents:(UIControlEventTouchUpInside)];
    }
    
}
- (void)textChange:(UITextField *)textFiled{
    if (self.phoneField.text.length >= 11 && self.passwordField.text.length >= 6 && self.checkCodeField.text.length > 0) {
        self.registerBtn.enabled = YES;
        self.registerBtn.backgroundColor = kUIColorFromRGB(0x01a1ff);
    }else{
...
- (void)hiddenPasswordAction:(UIButton *)sender{
